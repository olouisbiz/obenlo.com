            ...
            <div class="listing-sidebar">
                <div class="booking-widget">
                    ...
                    <?php else : ?>
                        <?php
                        // ── Booking Config Map ─────────────────────────────────
                        // Maps listing sub-type slug -> booking behaviour.
                        // Modes: date_range | datetime | event_datetime | date_only
                        $booking_config = array(
                            // Stay
                            'stay'               => array( 'mode' => 'date_range',     'price_unit' => '/ night',       'date_label' => 'Check-in / Check-out', 'has_guests' => true ),
                            'hotel'              => array( 'mode' => 'date_range',     'price_unit' => '/ night',       'date_label' => 'Check-in / Check-out', 'has_guests' => true ),
                            'guest-house'        => array( 'mode' => 'date_range',     'price_unit' => '/ night',       'date_label' => 'Check-in / Check-out', 'has_guests' => true ),
                            // Experience
                            'experience'         => array( 'mode' => 'date_only',      'price_unit' => '/ per person',  'date_label' => 'Select Date',          'has_guests' => true ),
                            'tour'               => array( 'mode' => 'date_only',      'price_unit' => '/ per person',  'date_label' => 'Tour Date',            'has_guests' => true ),
                            // Services (appointment-based, no guest count)
                            'service'            => array( 'mode' => 'datetime',       'price_unit' => '/ session',     'date_label' => 'Date & Time',          'has_guests' => false ),
                            'chauffeur'          => array( 'mode' => 'datetime',       'price_unit' => '/ hour',        'date_label' => 'Pick-up Date & Time',  'has_guests' => false ),
                            'cook'               => array( 'mode' => 'datetime',       'price_unit' => '/ session',     'date_label' => 'Appointment Date & Time', 'has_guests' => false ),
                            'barbershop'         => array( 'mode' => 'datetime',       'price_unit' => '/ appointment', 'date_label' => 'Appointment Date & Time', 'has_guests' => false ),
                            'hairdresser'        => array( 'mode' => 'datetime',       'price_unit' => '/ appointment', 'date_label' => 'Appointment Date & Time', 'has_guests' => false ),
                            'concierge'          => array( 'mode' => 'datetime',       'price_unit' => '/ session',     'date_label' => 'Date & Time',          'has_guests' => false ),
                            'personal-assistant' => array( 'mode' => 'datetime',       'price_unit' => '/ hour',        'date_label' => 'Date & Time',          'has_guests' => false ),
                            'babysitter'         => array( 'mode' => 'datetime',       'price_unit' => '/ hour',        'date_label' => 'Date & Time',          'has_guests' => false ),
                            'dogsitter'          => array( 'mode' => 'datetime',       'price_unit' => '/ day',         'date_label' => 'Date',                 'has_guests' => false ),
                            // Events (date + time, capacity = tickets)
                            'event'              => array( 'mode' => 'event_datetime', 'price_unit' => '/ ticket',      'date_label' => 'Event Date & Time',    'has_guests' => true ),
                            'show'               => array( 'mode' => 'event_datetime', 'price_unit' => '/ ticket',      'date_label' => 'Show Date & Time',     'has_guests' => true ),
                            // backwards-compat slugs (in case old terms exist)
                            'night-event'        => array( 'mode' => 'event_datetime', 'price_unit' => '/ ticket',      'date_label' => 'Event Date & Time',    'has_guests' => true ),
                            'performance-event'  => array( 'mode' => 'event_datetime', 'price_unit' => '/ ticket',      'date_label' => 'Event Date & Time',    'has_guests' => true ),
                        );

                        // Resolve config: try slug, then parent slug, then fallback
                        $cfg = isset( $booking_config[ $category ] ) ? $booking_config[ $category ] : null;
                        if ( ! $cfg && taxonomy_exists( 'listing_type' ) ) {
                            // Get parent term slug
                            $term_obj  = !empty($type_terms) ? get_term_by('name', $type_terms[0], 'listing_type') : null;
                            $parent_cfg_key = '';
                            if ( $term_obj && ! is_wp_error( $term_obj ) && $term_obj->parent ) {
                                $parent_term = get_term( $term_obj->parent, 'listing_type' );
                                $parent_cfg_key = ( $parent_term && ! is_wp_error( $parent_term ) ) ? $parent_term->slug : '';
                            }
                            $cfg = isset( $booking_config[ $parent_cfg_key ] ) ? $booking_config[ $parent_cfg_key ] : array( 'mode' => 'date_range', 'price_unit' => '/ unit', 'date_label' => 'Dates', 'has_guests' => true );
                        }
                        if ( ! $cfg ) {
                        ...
                        <form class="booking-form" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>" method="post">
                            ...
                            <?php elseif ( $booking_mode === 'datetime' ) : ?>
                                <div class="form-row">
                                    <label style="display: block; font-size: 0.9em; font-weight: bold; margin-bottom: 5px;"><?php echo esc_html($date_label); ?></label>
                            ...
                            <?php elseif ( $booking_mode === 'event_datetime' ) : ?>
                                <div class="form-row">
                                    <label style="display: block; font-size: 0.9em; font-weight: bold; margin-bottom: 5px;"><?php echo esc_html($date_label); ?></label>
                                    <input type="datetime-local" name="start_date" required style="width: 100%; border: 1px solid #ccc; padding: 10px; border-radius: 6px;" min="<?php echo date('Y-m-d\TH:i'); ?>">
                                </div>
                            ...
                            <?php if ( $form_has_guests ) : ?>
                                <div class="form-row">
                                    <label style="display: block; font-size: 0.9em; font-weight: bold; margin-bottom: 5px;">
                                        ...
                                        <?php if ($capacity) echo '(max ' . esc_html($capacity) . ')'; ?>
                                    </label>
                                    <input type="number" name="guests" min="1" <?php if ($capacity) echo 'max="' . esc_attr($capacity) . '"'; ?> value="1" required style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 6px;">
                                </div>
                            <?php else : ?>
                        ...
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            function calculateTotal() {
                                ...
                                } else if (bookingMode === 'date_only' || bookingMode === 'event_datetime' || bookingMode === 'datetime') {
