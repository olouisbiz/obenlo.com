...
if ( isset( $_POST['action'] ) && $_POST['action'] === 'update_profile' ) {
    if ( isset( $_POST['profile_nonce'] ) && wp_verify_nonce( $_POST['profile_nonce'], 'update_user_profile' ) ) {
        ...
        if ( is_wp_error( $user_id ) ) {
            wp_safe_redirect( add_query_arg( 'obenlo_error', 'update_failed', home_url('/account') ) );
        } else {
            wp_safe_redirect( add_query_arg( 'obenlo_message', '1', home_url('/account') ) );
        }
        exit;
    } else {
        wp_safe_redirect( add_query_arg( 'obenlo_error', 'security_failed', home_url('/account') ) );
        exit;
    }
}
...
<div class="obenlo-account-container listing-layout" style="max-width: 1200px; margin: 60px auto; padding: 0 20px; display: flex; gap: 40px; min-height: 600px;">
    ...
    <div class="listing-sidebar" style="width: 250px; flex-shrink: 0; display: flex; flex-direction: column; gap: 8px;">
        <h1 style="font-size: 2rem; font-weight: 800; margin: 0 0 20px 0; color: #222;"><?php echo __('Account', 'obenlo'); ?></h1>
        
        <a href="<?php echo home_url('/account/profile'); ?>" style="padding: 12px 18px; border-radius: 12px; text-decoration: none; font-weight: 700; color: <?php echo $tab === 'profile' ? '#fff' : '#666'; ?>; background: <?php echo $tab === 'profile' ? 'var(--obenlo-primary)' : 'transparent'; ?>; transition: all 0.2s;" onmouseover="if('<?php echo $tab; ?>'!=='profile')this.style.background='#f5f5f5'" onmouseout="if('<?php echo $tab; ?>'!=='profile')this.style.background='transparent'">
            <?php echo __('Personal Info', 'obenlo'); ?>
        </a>
        <a href="<?php echo home_url('/account/trips'); ?>" style="padding: 12px 18px; border-radius: 12px; text-decoration: none; font-weight: 700; color: <?php echo $tab === 'trips' ? '#fff' : '#666'; ?>; background: <?php echo $tab === 'trips' ? 'var(--obenlo-primary)' : 'transparent'; ?>; transition: all 0.2s;" onmouseover="if('<?php echo $tab; ?>'!=='trips')this.style.background='#f5f5f5'" onmouseout="if('<?php echo $tab; ?>'!=='trips')this.style.background='transparent'">
            <?php echo __('My Trips & Bookings', 'obenlo'); ?>
        </a>
        <a href="<?php echo home_url('/account/messages'); ?>" style="padding: 12px 18px; border-radius: 12px; text-decoration: none; font-weight: 700; color: <?php echo $tab === 'messages' ? '#fff' : '#666'; ?>; background: <?php echo $tab === 'messages' ? 'var(--obenlo-primary)' : 'transparent'; ?>; transition: all 0.2s;" onmouseover="if('<?php echo $tab; ?>'!=='messages')this.style.background='#f5f5f5'" onmouseout="if('<?php echo $tab; ?>'!=='messages')this.style.background='transparent'">
            <?php echo __('Messages', 'obenlo'); ?>
        </a>
        <a href="<?php echo home_url('/account/support'); ?>" style="padding: 12px 18px; border-radius: 12px; text-decoration: none; font-weight: 700; color: <?php echo $tab === 'support' ? '#fff' : '#666'; ?>; background: <?php echo $tab === 'support' ? 'var(--obenlo-primary)' : 'transparent'; ?>; transition: all 0.2s;" onmouseover="if('<?php echo $tab; ?>'!=='support')this.style.background='#f5f5f5'" onmouseout="if('<?php echo $tab; ?>'!=='support')this.style.background='transparent'">
            <?php echo __('Help & Support', 'obenlo'); ?>
        </a>
        <a href="<?php echo home_url('/account/broadcasts'); ?>" style="padding: 12px 18px; border-radius: 12px; text-decoration: none; font-weight: 700; color: <?php echo $tab === 'broadcasts' ? '#fff' : '#666'; ?>; background: <?php echo $tab === 'broadcasts' ? 'var(--obenlo-primary)' : 'transparent'; ?>; transition: all 0.2s;" onmouseover="if('<?php echo $tab; ?>'!=='broadcasts')this.style.background='#f5f5f5'" onmouseout="if('<?php echo $tab; ?>'!=='broadcasts')this.style.background='transparent'">
            <?php echo __('Announcements', 'obenlo'); ?>
        </a>
        ...
        <?php if ( in_array('host', (array)$user->roles) ) : ?>
            <div style="margin-top: 20px; border-top: 1px solid #eee; padding-top: 20px;">
                <a href="<?php echo esc_url( home_url('/host-dashboard') ); ?>" style="color: #222; font-weight: 700; text-decoration: none; display: flex; align-items: center; gap: 8px;">
                    ...
                    <?php echo __('Switch to Host Dashboard', 'obenlo'); ?>
                </a>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div style="flex-grow: 1; min-width: 0;">
        ...
        <?php if ( $tab === 'profile' ) : ?>
            <h2 style="font-size: 1.8rem; font-weight: 800; margin-bottom: 30px;"><?php echo __('Personal Info', 'obenlo'); ?></h2>
            ...
            <form method="POST" action="" style="max-width: 600px; background: #fff; padding: 30px; border: 1px solid #eee; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
                ...
                <div class="grid-row" style="margin-bottom: 20px;">
                    <div class="grid-col-1-2">
                        <label style="display: block; font-weight: 700; margin-bottom: 8px; color: #444;"><?php echo __('First Name', 'obenlo'); ?></label>
                    ...
                    <div class="grid-col-1-2">
                        <label style="display: block; font-weight: 700; margin-bottom: 8px; color: #444;"><?php echo __('Last Name', 'obenlo'); ?></label>
                ...
                <div style="margin-bottom: 20px;">
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: #444;"><?php echo __('Unique Guest ID', 'obenlo'); ?></label>
                    ...
                    <p style="font-size: 0.75rem; color: #888; margin-top: 6px;"><?php echo __('Use this ID for check-ins at the door or when contacting support.', 'obenlo'); ?></p>
                </div>

                <div style="margin-bottom: 30px;">
                    <label style="display: block; font-weight: 700; margin-bottom: 8px; color: #444;"><?php echo __('Email Address', 'obenlo'); ?></label>
                ...
                <button type="submit" style="background: #222; color: #fff; border: none; padding: 14px 30px; border-radius: 12px; font-weight: 700; font-size: 1rem; cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='#000'" onmouseout="this.style.background='#222'">
                    <?php echo __('Save Changes', 'obenlo'); ?>
                </button>
            </form>
        ...
        <?php elseif ( $tab === 'trips' ) : ?>
            ...
            <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 30px;">
                <h2 style="font-size: 1.8rem; font-weight: 800; margin: 0;"><?php echo __('My Trips & Bookings', 'obenlo'); ?></h2>
                <a href="<?php echo esc_url( home_url('/listings') ); ?>" style="color: var(--obenlo-primary); font-weight: 700; text-decoration: none;"><?php echo __('Explore Listings &rarr;', 'obenlo'); ?></a>
            </div>
            ...
            <?php if ( empty( $bookings ) ) : ?>
                <div style="text-align: center; padding: 60px 40px; background: #fff; border: 1px solid #eee; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.02);">
                    ...
                    <p style="color: #888; font-size: 0.95rem; margin: 0;"><?php echo __('Your booking history will appear here once you embark on a trip.', 'obenlo'); ?></p>
                </div>
            <?php else : ?>
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 20px; flex-wrap: wrap;">
                                    <div style="min-width: 0; flex: 1;">
                                        <div style="margin-bottom: 8px;">
                                            <span style="background: <?php echo $status_bg; ?>; color: <?php echo $status_color; ?>; font-size: 0.72rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.8px; padding: 4px 10px; border-radius: 20px;"><?php echo esc_html( __( ucwords( str_replace('_', ' ', $status) ), 'obenlo' ) ); ?></span>
                                        </div>
                                        ...
                                        <div style="color: #888; font-size: 0.88rem; margin-top: 4px;">
                                            ...
                                            <?php if ($guests) : ?>&nbsp;&bull;&nbsp;<?php printf(esc_html__('%s guest(s)', 'obenlo'), esc_html($guests)); ?><?php endif; ?>
                                        </div>
                                    </div>
                                        ...
                                        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; margin-top: 10px;">
                                            ...
                                            ?>
                                                <button onclick="if(window.obenloStartChatWith){window.obenloStartChatWith(<?php echo $host_id; ?>, '<?php echo esc_js($host_name); ?>', '<?php echo esc_url($host_avatar); ?>');} else { window.location.href='<?php echo esc_url(home_url('/login')); ?>'; }"
                                                    ...
                                                    <?php echo __('Message Host', 'obenlo'); ?>
                                                </button>
                                            <?php endif; endif; ?>
                                        </div>
                                    </div>
                                ...
                                <?php if ( $conf_code ) : ?>
                                    <div style="margin-top: 20px; padding: 15px; background: #fafafa; border: 1px solid #eee; border-radius: 12px; display: flex; flex-direction: column; gap: 12px;">
                                        <div style="display: flex; justify-content: space-between; align-items: center;">
                                            <div>
                                                <div style="font-size: 0.7rem; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: var(--obenlo-primary); margin-bottom: 2px;">🎫 <?php echo __('Confirmation Code', 'obenlo'); ?></div>
                                            ...
                                            <button onclick="navigator.clipboard.writeText('<?php echo esc_js($conf_code); ?>').then(()=>this.innerText='Copied!')" style="background: #fff; color: var(--obenlo-primary); border: 1px solid var(--obenlo-primary); padding: 6px 12px; border-radius: 8px; font-weight: 700; cursor: pointer; font-size: 0.75rem;"><?php echo __('Copy', 'obenlo'); ?></button>
                                        </div>
                                        <?php 
        ...
        <?php elseif ( $tab === 'broadcasts' ) : ?>
            <h2 style="font-size: 1.8rem; font-weight: 800; margin-bottom: 30px;"><?php echo __('Announcements', 'obenlo'); ?></h2>
            <?php Obenlo_Booking_Communication::render_broadcasts_list('guest'); ?>
