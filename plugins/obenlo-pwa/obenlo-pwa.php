...
define('OBENLO_PWA_VERSION', '1.6.8');
...
class Obenlo_PWA
{
    ...
    public function inject_pwa_script()
    {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // PWA Admin Hub: Inject link if user has admin privileges
            <?php if (current_user_can('administrator')): ?>
            const dashboardSidebar = document.querySelector('.dashboard-sidebar');
            if (dashboardSidebar) {
                const adminLink = document.createElement('a');
                adminLink.href = '<?php echo esc_url(admin_url("admin.php?page=obenlo-admin-dashboard")); ?>';
                adminLink.className = 'sidebar-link pwa-admin-link';
                adminLink.style.border = '2px solid #e61e4d';
                ...
                adminLink.innerHTML = `
                    ...
                    <span>Admin Hub</span>
                `;
                dashboardSidebar.appendChild(adminLink);
            }
            <?php endif; ?>

            // Detect if we are in the Admin Hub or using Admin settings and apply CSS class
            if (window.location.href.includes('obenlo-admin-dashboard') || window.location.href.includes('demo=1')) {
                document.body.classList.add('pwa-admin-hub');
                const container = document.querySelector('.obenlo-dashboard-container');
